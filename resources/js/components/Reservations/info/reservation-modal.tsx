...
import ReservationModalSkeleton from '@/components/Reservations/skeletons/reservation-modal-skeleton';
import {
    Dialog,
    DialogContent,
    DialogFooter,
...
function ReservationDetailsContent({ reservationId }: { reservationId: number }) {
    ...
    return (
        <>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500 dark:text-slate-400 font-medium">
                            {user.is_admin && (
                                <Badge variant="outline">Créé par : {reservation.creator?.name}</Badge>
                            )}
                            <Badge variant="outline">Créé le : {format(new Date(reservation.created_at), 'dd/MM/yyyy HH:mm') || 'N/A'}</Badge>
                        </div>
                    </div>
                </div>
            </DialogHeader>
            ...
            {/* Main Content Area */}
            <ScrollArea className="px-8 max-h-[60vh] p-3 space-y-3">
                <div className="space-y-2">
                    {/* Section: Visiteurs */}
